...
// Add this helper function to calculate end time
const calculateEndTime = (startTime) => {
   ...
   const formatDateFrench = (date) => {
    ...
    // Fonction pour vérifier si l'heure est déjà passée aujourd'hui
    const isTimeInPast = (time) => {
        if (!selectedDate || !isToday(selectedDate)) return false; // Si ce n'est pas aujourd'hui, toutes les heures sont libres
                        ...
                        <div>
                            ...
                            {selectedDate ? (
                                <div style={{
                                    ...
                                    {timeSlots.map((time) => {
                                        const pastTime = isTimeInPast(time); // On vérifie si l'heure est dépassée
                                        ...
                                        <button
                                            ...
                                            style={{
                                                ...
                                                transition: 'all 0.3s',
                                                opacity: (isSubmitting || pastTime) ? 0.3 : 1, // L'heure passée devient très transparente
                                                display: 'flex',
                                                alignItems: 'center',
                                                justifyContent: 'center',
                                                gap: '6px',
                                                textDecoration: pastTime ? 'line-through' : 'none' // Barre l'heure si elle est passée
                                            }}
                                            onMouseEnter={(e) => {
